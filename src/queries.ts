...
export const createSpriteImage = async (blobId: string, description: string) => {
...
export const getSpriteImages = async (first: number, after?: string) => {
  const options = {
...
export const getSprites = async (first: number, notInSpriteIds: string[], after?: string) => {
  const notInSpriteIdsStr = Array.from(notInSpriteIds)
    .map((sprite) => `"${sprite}"`)
    .join();
  ...
  const options = {
    ...
    orderBy: `timestamp`,
    orderDirection: `ASC`,
    meta: `{ documentId: { notIn: [${notInSpriteIdsStr}] } }`,
    fields: `{
      cursor
      fields {
        colour
